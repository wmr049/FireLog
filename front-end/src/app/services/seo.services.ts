...
export class SeoService {
    ...
    private setTitle(newTitle: string) {
        ...
        this.titleService.setTitle(newTitle + ' - Gerenciador de Logs');
    }

    private setMetaDescription(description: string) {
        this.metaDescription = this.getOrCreateMetaElement('description');
        if (StringUtils.isNullOrEmpty(description)) { description = 'Sistema para Gerenciar Logs'; }
        this.metaDescription.setAttribute('content', description);
    }

    private setMetaKeywords(keywords: string) {
        this.metaKeywords = this.getOrCreateMetaElement('keywords');
        if (StringUtils.isNullOrEmpty(keywords)) { keywords = 'Errors,Log,crash,gerenciar,.net,devops'; }
